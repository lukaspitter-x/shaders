...
- **GPU shader perf has cliffs, not slopes.** Constant-bound loops over a local array are fast only while the compiler fully unrolls them (array stays in registers); past a size/iteration threshold the array spills to memory and frame time jumps 3–4× at once. Symptoms: stubbing individual ALU costs changes nothing, but lowering the loop constant fixes it. → Bisect on the loop constant first, keep a same-run control (the last committed version) because GPU load from other tabs skews absolute numbers, and measure before "optimising" transcendentals — hardware `sin` beat trig-free hashes on Apple GPUs.
